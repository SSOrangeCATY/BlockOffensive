...
public final class CSSpectatorHudOverlay {
    ...
    // 名片纹理相关
    private static volatile Identifier currentCardLocation = null;
    private static volatile NameCardTexture currentCardTexture = null;
    private static volatile Identifier previousCardLocation = null;
    private static volatile NameCardTexture previousCardTexture = null;

    // 名片缓存 - 避免重复加载
    private static final Map<UUID, Identifier> CARD_LOCATION_CACHE = new ConcurrentHashMap<>();
    ...
    private void loadNameCardAsync(UUID targetUuid, Minecraft minecraft) {
        NAME_CARD_EXECUTOR.execute(() -> {
            // 检查缓存
            Identifier cachedLoc = CARD_LOCATION_CACHE.get(targetUuid);
            ...
            if (namecardFile != null && namecardFile.isFile()) {
                Identifier textureId = Identifier.tryBuild(FPSMatch.MODID, "namecard/" + targetUuid);
     ...
     * 应用名片到当前显示
     */
    private static void applyNamecard(UUID owner, Identifier id, NameCardTexture tex) {
     ...
     * 渲染面板背景
     */
    private void renderPanelBackground(GuiGraphicsExtractor g, int x, int y, int width, int height, float alpha) {
        // 绘制背景和高光
     ...
     * 渲染名片层
     */
    private void renderCardLayer(GuiGraphicsExtractor g, int x, int y, int width, int height,
                                 Identifier loc, NameCardTexture tex, float alpha) {
        ...
        g.pose().translate(x, y + yOffset);
        g.pose().scale(scale, scale);

        int color = mulAlpha(0xFFFFFFFF, alpha);

        // 绘制卡片（适配高度）
        if (scaledHeight <= height) {
            g.blit(RenderPipelines.GUI_TEXTURED, loc, 0, 0, 0.0F, 0.0F, tex.realWidth, tex.realHeight, tex.realWidth, tex.realHeight, color);
        } else {
            float uvTop = (scaledHeight - height) / 2f / scaledHeight;
            float uvBottom = 1f - uvTop;
            int drawHeight = (int) (height / scale);
            g.blit(RenderPipelines.GUI_TEXTURED, loc, 0, 0,
                    0.0F, uvTop * tex.realHeight,
                    tex.realWidth, drawHeight,
                    tex.realWidth, (int) ((uvBottom - uvTop) * tex.realHeight),
                    tex.realWidth, tex.realHeight, color);
        }
     ...
     * 渲染团队条纹
     */
    private void renderTeamStripes(GuiGraphicsExtractor g, int x, int y, int width, int height, float alpha) {
     ...
     * 渲染玩家详细信息（头像、名称、状态等）
     */
    private void renderPlayerInfo(GuiGraphicsExtractor g, Minecraft mc, int x, int y, int width, int height, float alpha) {
     ...
     * 渲染玩家头像
     */
    private void renderAvatar(GuiGraphicsExtractor g, Minecraft mc, int centerX, int centerY, float alpha) {
        ...
        g.pose().pushMatrix();
        g.pose().translate(avatarX, avatarY);
        g.pose().scale(avatarSize / 8f, avatarSize / 8f);
        g.blit(RenderPipelines.GUI_TEXTURED, lastTargetPlayer.getSkin().body().texturePath(), 0, 0, 8.0F, 8.0F, 8, 8, 64, 64);
        g.pose().popMatrix();
    }
     ...
     * 渲染玩家名称
     */
    private void renderPlayerName(GuiGraphicsExtractor g, Minecraft mc, int leftX, int centerY, float alpha) {
        ...
        g.pose().pushMatrix();
        g.pose().translate(leftX - (int) (mc.font.width(playerName) * nameScale), textY);
        g.pose().scale(nameScale, nameScale);
        g.text(mc.font, playerName, 0, 0, mulAlpha(TEXT_COLOR_WHITE, alpha), false);
        g.pose().popMatrix();
    }
     ...
     * 渲染物品信息
     */
    private void renderItemInfo(GuiGraphicsExtractor g, Minecraft mc, int leftX, int centerY, float alpha) {
        ...
        g.text(mc.font, itemNameCn, leftX - mc.font.width(itemNameCn), cnY,
                mulAlpha(TEXT_COLOR_ITEM, alpha), false);
        g.text(mc.font, itemNameEn, leftX - mc.font.width(itemNameEn), enY,
     ...
     */
    private void renderPlayerStats(GuiGraphicsExtractor g, Minecraft mc, int rightX, int centerY, int panelWidth, float alpha) {
        ...
        g.text(mc.font, headshotText, rightX, startY,
                mulAlpha(TEXT_COLOR_HEADSHOT, alpha), false);

        // 渲染击杀数
        int kills = Math.round(shownKills);
        g.text(mc.font, String.format(FORMAT_KILLS, kills), rightX, startY + lineHeight,
     ...
     * 渲染生命值条
     */
    private void renderHealthBar(GuiGraphicsExtractor g, Minecraft mc, int x, int y, int panelWidth, float alpha) {
