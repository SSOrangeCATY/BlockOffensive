...
public class C4TimerHUD {
    ...
    public void render(GuiGraphics guiGraphics, int screenWidth, int screenHeight) {
        ...
        int totalFuseTicks = Math.max(1, CSClientData.bombTotalFuse);
        int remainingTicks = Math.min(CSClientData.bombFuse, totalFuseTicks);
        ...
        renderHUD(guiGraphics, minecraft, hudX, hudY, hudWidth, hudHeight, remainingSeconds, remainingTicks, totalFuseTicks);
        lastSeconds = remainingSeconds;
    }

    private boolean shouldRenderHud() {
        return CSClientData.isStart && !CSClientData.isWaitingWinner && !CSClientData.isPause && !CSClientData.isWaiting
                && !CSClientData.isWarmTime && CSClientData.bombFuse > 0;
    }

    private void renderHUD(GuiGraphics guiGraphics, Minecraft minecraft, int hudX, int hudY, int hudWidth, int hudHeight,
                           int remainingSeconds, int remainingTicks, int totalFuseTicks) {
        int bgColor;
        int lineColor;
        int textColor;
